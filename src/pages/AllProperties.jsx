...
const AllProperties = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://home-nest-server-ashen.vercel.app/all-properties?price=${sortPrice}`)
      .then((res) => {
        setData(res.data);
      })
  ...
  const searchedProperty = term
    ? data?.filter((data) =>
        data.propertyName.toLocaleLowerCase().includes(term)
      )
    : data;
  return (
    <div className="px-6 md:px-10 lg:px-20 py-10 lg:py-20 space-y-4 md:space-y-10">
      <div className="space-y-4">
        ...
        <p className="text-center text-[#627382] text-sm md:text-lg">
          Explore our wide range of properties available for rent, sale, and
          commercial use. Find your perfect home or investment opportunity with
          ease.
        </p>
      </div>
        ...
        <div>
          <div className="flex flex-col text-center items-center justify-center h-screen p-10 md:p-20 space-y-4">
            ...
            <p className="text-sm md:text-xl text-[#627382]">
              The Property you are requesting is not found on our system. please
