...
const MyRatings = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://home-nest-server-ashen.vercel.app/my-ratings?email=${userEmail}`)
      .then((res) => {
        setRatings(res.data);
      })
      .catch((err) => {
    ...
    return (
      <div className="flex flex-col text-center items-center justify-center h-screen p-10 md:p-20 space-y-4">
        ...
        <p className="text-sm md:text-xl text-[#627382]">
          Explore properties and share your feedback by submitting ratings!
        </p>
      </div>
    );
  }
  return (
    <div className="px-6 md:px-10 lg:px-20 py-10 lg:py-20 space-y-4 md:space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {ratings?.map((rate) => (
          <RatingCard key={rate._id} rate={rate}></RatingCard>
        ))}
      </div>
    </div>
  );
